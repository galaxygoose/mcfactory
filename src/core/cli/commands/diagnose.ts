...
export function diagnoseCommand(): void {
  console.log('🔍 MCF System Diagnostics\n');
  ...
  results.push({
    ...
    message: `Node.js ${nodeVersion} ${majorVersion >= 18 ? '(✓)' : '(✗ requires >=18)'}`
  });
  ...
  results.push({
    ...
    message: configExists ? `Found ${CONFIG_PATH}` : `Missing ${CONFIG_PATH} (run "mcfactory init")`
  });
  ...
  if (failCount > 0) {
    ...
    console.log(`⚠️  ${warnCount} warning(s) - system is functional but may need configuration`);
  } else {
