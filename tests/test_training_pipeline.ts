...
async function testTrainingPipeline() {
  ...
  try {
    ...
    console.log('📊 Step 1: Collecting training data from factories...');
    const examples = collectFromFactories();
    console.log(`✅ Collected ${examples.length} training examples\n`);

    // Step 2: Generate labels
    console.log('🏷️  Step 2: Generating labels...');
    const labeledExamples = generateLabels(examples);
    console.log(`✅ Generated labels for ${labeledExamples.length} examples\n`);
    ...
    if (!validationResult.valid) {
      console.log('❌ Dataset validation failed. Stopping test.');
      return;
    }

    // Step 4: Apply sharding
    console.log('🗂️  Step 4: Applying dataset sharding...');
    const shardedData = manageShards(labeledExamples, { maxShardSize: 5 });
    console.log(`✅ Created ${shardedData.shards.length} shards`);
    ...
    console.log(`✅ Parquet format: ${parquetData.rows.length} rows, ${parquetData.schema.fields.length} fields\n`);
    ...
    try {
      ...
      if (!apiKey) {
        console.log('⚠️  OPENAI_API_KEY not found in .env file. Skipping embedding generation.\n');
      } else {
        console.log('🔑 OpenAI API key found. Generating embeddings...');
        const embeddingResult = await generateEmbeddingsForExamples(
          shardedData.shards[0].slice(0, 2),
          { provider: 'embedding', fields: ['input'] }
        );
        console.log(`✅ Generated embeddings for ${embeddingResult.embeddings.length} examples`);
        console.log(`   - Dimensions: ${embeddingResult.dimensions}\n`);
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.log('❌ Embedding generation failed:', errorMessage);
      console.log('   (This may be due to API rate limits or connectivity issues)\n');
    }

    // Step 7: Generate manifest
    console.log('📋 Step 7: Generating training manifest...');
    const manifest = generateManifest(labeledExamples, {
      datasetId: 'test_dataset',
      description: 'Test dataset generated by MCF training pipeline',
      includeExamples: false
    });
    console.log(`✅ Generated manifest for dataset: ${manifest.datasetId}`);
    ...
    console.log('📅 Step 8: Testing export scheduler...');
    const jobId = scheduleExport('Test Export Job', 'manual', {
      format: 'jsonl',
      shardSize: 5,
      schedule: 'manual'
    });
    console.log(`✅ Scheduled export job: ${jobId}`);

    // Run the export
    console.log('🚀 Running export...');
    const exportResult = await runExport(jobId);
    console.log(`✅ Export ${exportResult.success ? 'SUCCEEDED' : 'FAILED'}`);
    ...
    console.log(`🏷️  Labeled Examples: ${labeledExamples.length}`);
    console.log(`🗂️  Shards Created: ${shardedData.shards.length}`);
    console.log(`📄 Files Generated: ${exportResult.files.length}`);
    console.log(`✅ Pipeline Status: SUCCESS`);

  } catch (error) {
    console.error('❌ Training pipeline test failed:', error);
    process.exit(1);
  }
}
